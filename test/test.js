...
//                                                                                  CMD   CMDV        NumChannels   SubLen  Status  Channel     Type  Value                     SubLen  Status  Channel     Type  Value       ETX   CRC         ETX                                    
...
//                                                                                  CMD   CMDV        NumChannels   SubLen  Status  Channel     Type  Value                     SubLen  Status  Channel     Type  Value                                           SubLen    Status  Channel     Type    Value                   SubLen  Status  Channel     Type    Value       SubLen  Status  Channel     Type    Value       ETX   CRC         ETO
...
//                                                                                  CMD   CMDV        NumChannels   SubLen  Status  Channel     Type Value                      ETX     CRC         ETO
let onlineCmdResponse_part_1_2  = [ 0x01, 0x10, 0x01, 0xf0, 0x01, 0x70, 0x0d, 0x02, 0x2f, 0x10, 0x00, 0x01,         0x08,   0x00,   0x64, 0x00, 0x16, 0x54, 0xd7, 0xb7 ];
let onlineCmdResponse_part_2_2  = [                                                                                                                                       0x41, 0x03,   0x19, 0x4a, 0x04 ];
...
let umbparser = new mod_umbparser.UMBParser(new dummyNode());
...
describe('#online command', function(){
    ...
    it('Online command data with 5 ok values', function() {
        ...
        expChValues[0].ch_value = 42.49283981323242;
        ...
        umbresponse.umbframe.framedata.raw.forEach(parsedCh => {
    ...
    it('Online command data with segemented data', function() {
        umbresponse = umbparser.ParseReadBuf(onlineCmdResponse_part_1_2);
        assert.equal(umbresponse.parserState, "processing", "Parser should be in processing state after first Chunk");
        umbresponse = umbparser.ParseReadBuf(onlineCmdResponse_part_2_2);
        assert.equal(umbresponse.parserState, "finished", "Parser didn't finish");
        assert.equal(umbresponse.umbframe.status, 0x00, "Status !0 OK");
        assert.equal(umbresponse.umbframe.cmd, 0x2F, "wrong command detected");        
    })
});
